...
Azure Data Studio ابزاری برای مدیریت Database است که
توسط مایکروسافت به صورت
Open Source
عرضه شده و روی ویندوز، مک و لینوکس کار می‌کند.
...
SQL Server
هم تکرار می‌کنید تا ببینید کجاها نوشتن SQL فرق می‌کند.
:::
...
1. اگر فردا
   MySQL، Oracle، SQLite
   و چند Database دیگر هم اضافه شود، برای هر کدام چه کار می‌کنید؟
1. اگر این queryها داخل کد
   #C
   به‌صورت
   `string`
   خام باشند، با هر Database جدید چه اتفاقی برای نگهداری کد می‌افتد؟
1. چطور می‌شود یک‌بار منطق query را تعریف کرد و فقط ترجمهٔ آن به SQL هر Database را جدا کرد؟

:::note ‌
این درد، دقیقاً همان جایی است که
Query Builder
به کار می‌آید: یک‌بار منطق query را می‌نویسید و برای هر Database جداگانه به SQL ترجمه می‌کنید.
:::
