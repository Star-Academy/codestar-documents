...
CRUD
ساده بسازید:
حدود **پنج عملیات** برای لیست کردن، گرفتن یکی، ساختن، به‌روزرسانی و حذف.
نام و مسیر
endpointها
را خودتان از روی مستندات
ASP.NET Core
(و قراردادهای
REST
فاز
Network)
انتخاب کنید؛ مهم رفتار درست است، نه یک اسم ثابت.
...
نکتهٔ اصلی: کلاینت در هر درخواست مشخص می‌کند روی کدام
Database
اجرا شود؛ سرور همان را انتخاب می‌کند و query را آنجا می‌زند.
مثلاً با query string یا header
(`db=postgres`
/
`db=sqlserver`).
connection string
هر دو را در تنظیمات برنامه بگذارید؛ فقط **انتخاب** از ورودی درخواست باشد.
اگر
Database
نامعتبر بود یا دانشجو پیدا نشد، پاسخ خطای واضح برگردانید.
...
1. انتخاب
   Database
   نامعتبر و Student پیدا‌نشده پاسخ خطای مشخص دارند.
1. queryها با
   SqlKata
   ساخته می‌شوند و برای هر
   Database
   Compiler
   درست انتخاب می‌شود.
