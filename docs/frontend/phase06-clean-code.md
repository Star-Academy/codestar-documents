...
در این فاز ما مجموعه‌ای از Best Practiceها و Anti-patternها را جمع‌آوری کرده‌ایم
...
این مجموعه را توسعه دهیم تا در آینده تبدیل به مرجع مناسبی برای شما و دیگر توسعه‌دهندگان شود.
...
و احساس کنید تقریباً هیچ‌چیز از کدی که نوشته‌اید متوجه نمی‌شوید.
زمانی که بر روی پروژه‌ای کوچک به صورت انفرادی کار می‌کنید
شاید هیچ‌وقت به اهمیت کد تمیز پی نبرید؛
اما اگر همین پروژه بعداً احتیاج به توسعه داشته باشید
...
کردن آن است، به صورتی که نشانه‌های کد کثیف از بین بروند. به این نشانه‌ها
...
-   لینک برای دانلود فایل نهایی/اجرایی پروژه
...
در دنیای Open-Source، آدم‌های زیادی پیدا می‌شوند که به طور رایگان بخواهد به شما کمک کنند؛
...
مرورگرهای مختلف، استایل‌های پیش‌فرض متفاوتی برای المان‌ها مختلف دارند
...
در CSS به طور کلی از 4 روش مختلف می‌توان رنگ یک المان را تعیین کرد که در زیر به مقایسه هر کدام می‌پردازیم.
...
آیا برای اثبات بد بودن این فرمت احتیاج به توضیح بیشتری دارید؟
...
مشکل این فرمت این است که نمی‌توان رنگ فعلی را به راحتی تشخیص داد.
...
میزان استفاده از رنگ پایه
و روشنایی رنگ نهایی را مشخص می‌کنند.
اگر Color Wheel و زاویه رنگ‌های اصلی را حفظ کنید،
...
همچنین پیشنهاد می‌کنیم متغیرها را در بالای بقیۀ Ruleها تعریف کنید تا خوانایی کد بالا رود و از اشتباهات احتمالی جلوگیری شود:
...
دقت کنید که در اینجا منظور فقط کلیدواژۀ `animation` یا `keyframe` نیست،
...
قطعاً متوجه شدید که نیاز است خیلی از استایل‌هایی که به راست یا چپ ربط دارند را عوض کنید.
...
Angular
برای استایل‌های Global شما فایل styles.scss را فراهم کرده؛
...
همچنین از این فایل برای تعریف کلاس‌های کمکی نیز می‌توانید استفاده کنید.

### Comment Dictionary

برخی از اشتباهات پُرتکرار هستند و کامنتی که برای رفع آن‌ها گذاشته می‌شود همیشه یکسان است.
برای این موارد، از علائم و اختصارات استفاده می‌کنیم که سرعت Review کد بیشتر شود.
...
|  نمیگم   | مشکلی که وجود دارد، قبلاً ذکر شده بود؛ برای جلوگیری از تعدد کامنت‌ها، این موضوع دیگر تذکر داده نمی‌شود، اما باید در تمام نقاط پروژه مورد بررسی قرار بگیرد و حل شود |
...
از یکی از تیم‌ها درخواست کنید تا کد شما را Review کنند؛
