...
**Change Detection**
را متحول کرده و منجر به بهبود چشمگیر عملکرد می‌شود.

### مفاهیم پایه‌ای

سیستم
Signals
بر سه مفهوم اصلی استوار است که با هم کار می‌کنند تا یک سیستم
Reactive
قدرتمند را شکل دهند.
...
یا
wrapper
برای یک مقدار است که می‌تواند به مرور زمان تغییر کند. این جعبه به هر
consumer
...
واکنش نشان می‌دهد.
`effect`
مستقیماً در تمپلیت استفاده نمی‌شوند و مقدار بازگشتی ندارند. آن‌ها برای هماهنگ‌سازی وضعیت اپلیکیشن شما با دنیای خارج از انگولار (مثل کنسول،
`localStorage`
...
(مثل
`constructor`
کامپوننت، دایرکتیو یا سرویس) ایجاد شود. این کار باعث می‌شود انگولار به طور خودکار
`effect`
را با از بین رفتن
(destroy)
...
`signal`
به طور پیش‌فرض برابری مقادیر را با اپراتور `===` (identity comparison) بررسی می‌کند. این یعنی برای آبجکت‌ها و آرایه‌ها،
`signal`
